...
public final class AggregateOperator extends SoleOutOperator implements MemoryRevoker {
    ...
    public ListenableFuture<?> startMemoryRevoke(AbstractParams param) {
        ...
        SpillManager.getSpillExecutor().execute(() -> {
            try {
                aggParams.spillAggCache();
                LogUtils.info(log, "AggregateOperator spilled current buffer during memory revocation");
                future.set(null);
            } catch (IOException e) {
                LogUtils.warn(log, "AggregateOperator failed to spill during memory revocation: {}", e.getMessage());
