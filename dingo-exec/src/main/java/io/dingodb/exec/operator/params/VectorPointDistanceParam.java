...
public class VectorPointDistanceParam extends AbstractParams implements RevokerParams {
    ...
    public void init(Vertex vertex) {
        ...
        // Initialize memory pool for spill support
        if (schema != null && ScopeVariables.enableSpill()) {
    ...
    /**
     * Returns a lazy iterator over all tuples: spill files first (in order), then in-memory cache.
     * Spill file streams are opened on demand and closed when exhausted.
     * Caller should invoke {@link #clear()} after iteration to delete spill files.
     *
     * @throws IOException if a spill file cannot be opened
     */
    public Iterator<Object[]> tupleIterator() throws IOException {
        if (!hasSpillFiles()) {
            return cache.iterator();
        }
        List<Iterator<Object[]>> iterators = new ArrayList<>(spillFiles.size() + 1);
        for (TupleSpillFile sf : spillFiles) {
            iterators.add(sf.iterator());
        }
        iterators.add(cache.iterator());
